...
const sendVerificationEmail = async (to, token) => {
  try {
    ...
    const verificationEmailUrl = `http://localhost:3000/api/auths/verify-email?token=${token}`;
...
const sendEmailResetPassword = async(to,token)=>{
  try {
    ...
    const verificationPasswordURL = `http://localhost:3000/api/auths/verify-passwod?email=${to}&token=${token}`;
